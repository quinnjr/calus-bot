/* 
 * Bungie.Net API
 *
 * These endpoints constitute the functionality exposed by Bungie.net, both for more traditional website functionality and for connectivity to Bungie video games and their related functionality.
 *
 * OpenAPI spec version: 2.7.1
 * Contact: support@bungie.com
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

/// DestinyDefinitionsDestinyTalentGridDefinition : The time has unfortunately come to talk about Talent Grids.  Talent Grids are the most complex and unintuitive part of the Destiny Definition data. Grab a cup of coffee before we begin, I can wait.  Talent Grids were the primary way that items could be customized in Destiny 1. In Destiny 2, for now, talent grids have become exclusively used by Subclass/Build items: but the system is still in place for it to be used by items should the direction change back toward talent grids.  Talent Grids have Nodes: the visual circles on the talent grid detail screen that have icons and can be activated if you meet certain requirements and pay costs. The actual visual data and effects, however, are driven by the \"Steps\" on Talent Nodes. Any given node will have 1:M of these steps, and the specific step that will be considered the \"current\" step (and thus the dictator of all benefits, visual state, and activation requirements on the Node) will almost always not be determined until an instance of the item is created. This is how, in Destiny 1, items were able to have such a wide variety of what users saw as \"Perks\": they were actually Talent Grids with nodes that had a wide variety of Steps, randomly chosen at the time of item creation.  Now that Talent Grids are used exclusively by subclasses and builds, all of the properties within still apply: but there are additional visual elements on the Subclass/Build screens that are superimposed on top of the talent nodes. Unfortunately, BNet doesn't have this data: if you want to build a subclass screen, you will have to provide your own \"decorative\" assets, such as the visual connectors between nodes and the fancy colored-fire-bathed character standing behind the nodes.  DestinyInventoryItem.talentGrid.talentGridHash defines an item's linked Talent Grid, which brings you to this definition that contains enough satic data about talent grids to make your head spin. These *must* be combined with instanced data - found when live data returns DestinyItemTalentGridComponent - in order to derive meaning. The instanced data will reference nodes and steps within these definitions, which you will then have to look up in the definition and combine with the instanced data to give the user the visual representation of their item's talent grid.

#[allow(unused_imports)]
use serde_json::Value;

#[derive(Debug, Serialize, Deserialize)]
pub struct DestinyDefinitionsDestinyTalentGridDefinition {
  /// The maximum possible level of the Talent Grid: at this level, any nodes are allowed to be activated.
  #[serde(rename = "maxGridLevel")]
  max_grid_level: Option<i32>,
  /// The meaning of this has been lost in the sands of time: it still exists as a property, but appears to be unused in the modern UI of talent grids. It used to imply that each visual \"column\" of talent nodes required identical progression levels in order to be activated. Returning this value in case it is still useful to someone? Perhaps it's just a bit of interesting history.
  #[serde(rename = "gridLevelPerColumn")]
  grid_level_per_column: Option<i32>,
  /// The hash identifier of the Progression (DestinyProgressionDefinition) that drives whether and when Talent Nodes can be activated on the Grid. Items will have instances of this Progression, and will gain experience that will eventually cause the grid to increase in level. As the grid's level increases, it will cross the threshold where nodes can be activated. See DestinyTalentGridStepDefinition's activation requirements for more information.
  #[serde(rename = "progressionHash")]
  progression_hash: Option<i32>,
  /// The list of Talent Nodes on the Grid (recall that Nodes themselves are really just locations in the UI to show whatever their current Step is. You will only know the current step for a node by retrieving instanced data through platform calls to the API that return DestinyItemTalentGridComponent).
  #[serde(rename = "nodes")]
  nodes: Option<Vec<::models::DestinyDefinitionsDestinyTalentNodeDefinition>>,
  /// Talent Nodes can exist in \"exclusive sets\": these are sets of nodes in which only a single node in the set can be activated at any given time. Activating a node in this set will automatically deactivate the other nodes in the set (referred to as a \"Swap\").  If a node in the exclusive set has already been activated, the game will not charge you materials to activate another node in the set, even if you have never activated it before, because you already paid the cost to activate one node in the set.  Not to be confused with Exclusive Groups. (how the heck do we NOT get confused by that? Jeez) See the groups property for information about that only-tangentially-related concept.
  #[serde(rename = "exclusiveSets")]
  exclusive_sets: Option<Vec<::models::DestinyDefinitionsDestinyTalentNodeExclusiveSetDefinition>>,
  /// This is a quick reference to the indexes of nodes that are not part of exclusive sets. Handy for knowing which talent nodes can only be activated directly, rather than via swapping.
  #[serde(rename = "independentNodeIndexes")]
  independent_node_indexes: Option<Vec<i32>>,
  /// Talent Nodes can have \"Exclusive Groups\". These are not to be confused with Exclusive Sets (see exclusiveSets property).  Look at the definition of DestinyTalentExclusiveGroup for more information and how they work. These groups are keyed by the \"groupHash\" from DestinyTalentExclusiveGroup.
  #[serde(rename = "groups")]
  groups: Option<::std::collections::HashMap<String, ::models::DestinyDefinitionsDestinyTalentExclusiveGroup>>,
  /// BNet wants to show talent nodes grouped by similar purpose with localized titles. This is the ordered list of those categories: if you want to show nodes by category, you can iterate over this list, render the displayProperties for the category as the title, and then iterate over the talent nodes referenced by the category to show the related nodes.  Note that this is different from Exclusive Groups or Sets, because these categories also incorporate \"Independent\" nodes that belong to neither sets nor groups. These are purely for visual grouping of nodes rather than functional grouping.
  #[serde(rename = "nodeCategories")]
  node_categories: Option<Vec<::models::DestinyDefinitionsDestinyTalentNodeCategory>>,
  /// The unique identifier for this entity. Guaranteed to be unique for the type of entity, but not globally.  When entities refer to each other in Destiny content, it is this hash that they are referring to.
  #[serde(rename = "hash")]
  hash: Option<i32>,
  /// The index of the entity as it was found in the investment tables.
  #[serde(rename = "index")]
  index: Option<i32>,
  /// If this is true, then there is an entity with this identifier/type combination, but BNet is not yet allowed to show it. Sorry!
  #[serde(rename = "redacted")]
  redacted: Option<bool>
}

impl DestinyDefinitionsDestinyTalentGridDefinition {
  /// The time has unfortunately come to talk about Talent Grids.  Talent Grids are the most complex and unintuitive part of the Destiny Definition data. Grab a cup of coffee before we begin, I can wait.  Talent Grids were the primary way that items could be customized in Destiny 1. In Destiny 2, for now, talent grids have become exclusively used by Subclass/Build items: but the system is still in place for it to be used by items should the direction change back toward talent grids.  Talent Grids have Nodes: the visual circles on the talent grid detail screen that have icons and can be activated if you meet certain requirements and pay costs. The actual visual data and effects, however, are driven by the \"Steps\" on Talent Nodes. Any given node will have 1:M of these steps, and the specific step that will be considered the \"current\" step (and thus the dictator of all benefits, visual state, and activation requirements on the Node) will almost always not be determined until an instance of the item is created. This is how, in Destiny 1, items were able to have such a wide variety of what users saw as \"Perks\": they were actually Talent Grids with nodes that had a wide variety of Steps, randomly chosen at the time of item creation.  Now that Talent Grids are used exclusively by subclasses and builds, all of the properties within still apply: but there are additional visual elements on the Subclass/Build screens that are superimposed on top of the talent nodes. Unfortunately, BNet doesn't have this data: if you want to build a subclass screen, you will have to provide your own \"decorative\" assets, such as the visual connectors between nodes and the fancy colored-fire-bathed character standing behind the nodes.  DestinyInventoryItem.talentGrid.talentGridHash defines an item's linked Talent Grid, which brings you to this definition that contains enough satic data about talent grids to make your head spin. These *must* be combined with instanced data - found when live data returns DestinyItemTalentGridComponent - in order to derive meaning. The instanced data will reference nodes and steps within these definitions, which you will then have to look up in the definition and combine with the instanced data to give the user the visual representation of their item's talent grid.
  pub fn new() -> DestinyDefinitionsDestinyTalentGridDefinition {
    DestinyDefinitionsDestinyTalentGridDefinition {
      max_grid_level: None,
      grid_level_per_column: None,
      progression_hash: None,
      nodes: None,
      exclusive_sets: None,
      independent_node_indexes: None,
      groups: None,
      node_categories: None,
      hash: None,
      index: None,
      redacted: None
    }
  }

  pub fn set_max_grid_level(&mut self, max_grid_level: i32) {
    self.max_grid_level = Some(max_grid_level);
  }

  pub fn with_max_grid_level(mut self, max_grid_level: i32) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.max_grid_level = Some(max_grid_level);
    self
  }

  pub fn max_grid_level(&self) -> Option<&i32> {
    self.max_grid_level.as_ref()
  }

  pub fn reset_max_grid_level(&mut self) {
    self.max_grid_level = None;
  }

  pub fn set_grid_level_per_column(&mut self, grid_level_per_column: i32) {
    self.grid_level_per_column = Some(grid_level_per_column);
  }

  pub fn with_grid_level_per_column(mut self, grid_level_per_column: i32) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.grid_level_per_column = Some(grid_level_per_column);
    self
  }

  pub fn grid_level_per_column(&self) -> Option<&i32> {
    self.grid_level_per_column.as_ref()
  }

  pub fn reset_grid_level_per_column(&mut self) {
    self.grid_level_per_column = None;
  }

  pub fn set_progression_hash(&mut self, progression_hash: i32) {
    self.progression_hash = Some(progression_hash);
  }

  pub fn with_progression_hash(mut self, progression_hash: i32) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.progression_hash = Some(progression_hash);
    self
  }

  pub fn progression_hash(&self) -> Option<&i32> {
    self.progression_hash.as_ref()
  }

  pub fn reset_progression_hash(&mut self) {
    self.progression_hash = None;
  }

  pub fn set_nodes(&mut self, nodes: Vec<::models::DestinyDefinitionsDestinyTalentNodeDefinition>) {
    self.nodes = Some(nodes);
  }

  pub fn with_nodes(mut self, nodes: Vec<::models::DestinyDefinitionsDestinyTalentNodeDefinition>) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.nodes = Some(nodes);
    self
  }

  pub fn nodes(&self) -> Option<&Vec<::models::DestinyDefinitionsDestinyTalentNodeDefinition>> {
    self.nodes.as_ref()
  }

  pub fn reset_nodes(&mut self) {
    self.nodes = None;
  }

  pub fn set_exclusive_sets(&mut self, exclusive_sets: Vec<::models::DestinyDefinitionsDestinyTalentNodeExclusiveSetDefinition>) {
    self.exclusive_sets = Some(exclusive_sets);
  }

  pub fn with_exclusive_sets(mut self, exclusive_sets: Vec<::models::DestinyDefinitionsDestinyTalentNodeExclusiveSetDefinition>) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.exclusive_sets = Some(exclusive_sets);
    self
  }

  pub fn exclusive_sets(&self) -> Option<&Vec<::models::DestinyDefinitionsDestinyTalentNodeExclusiveSetDefinition>> {
    self.exclusive_sets.as_ref()
  }

  pub fn reset_exclusive_sets(&mut self) {
    self.exclusive_sets = None;
  }

  pub fn set_independent_node_indexes(&mut self, independent_node_indexes: Vec<i32>) {
    self.independent_node_indexes = Some(independent_node_indexes);
  }

  pub fn with_independent_node_indexes(mut self, independent_node_indexes: Vec<i32>) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.independent_node_indexes = Some(independent_node_indexes);
    self
  }

  pub fn independent_node_indexes(&self) -> Option<&Vec<i32>> {
    self.independent_node_indexes.as_ref()
  }

  pub fn reset_independent_node_indexes(&mut self) {
    self.independent_node_indexes = None;
  }

  pub fn set_groups(&mut self, groups: ::std::collections::HashMap<String, ::models::DestinyDefinitionsDestinyTalentExclusiveGroup>) {
    self.groups = Some(groups);
  }

  pub fn with_groups(mut self, groups: ::std::collections::HashMap<String, ::models::DestinyDefinitionsDestinyTalentExclusiveGroup>) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.groups = Some(groups);
    self
  }

  pub fn groups(&self) -> Option<&::std::collections::HashMap<String, ::models::DestinyDefinitionsDestinyTalentExclusiveGroup>> {
    self.groups.as_ref()
  }

  pub fn reset_groups(&mut self) {
    self.groups = None;
  }

  pub fn set_node_categories(&mut self, node_categories: Vec<::models::DestinyDefinitionsDestinyTalentNodeCategory>) {
    self.node_categories = Some(node_categories);
  }

  pub fn with_node_categories(mut self, node_categories: Vec<::models::DestinyDefinitionsDestinyTalentNodeCategory>) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.node_categories = Some(node_categories);
    self
  }

  pub fn node_categories(&self) -> Option<&Vec<::models::DestinyDefinitionsDestinyTalentNodeCategory>> {
    self.node_categories.as_ref()
  }

  pub fn reset_node_categories(&mut self) {
    self.node_categories = None;
  }

  pub fn set_hash(&mut self, hash: i32) {
    self.hash = Some(hash);
  }

  pub fn with_hash(mut self, hash: i32) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.hash = Some(hash);
    self
  }

  pub fn hash(&self) -> Option<&i32> {
    self.hash.as_ref()
  }

  pub fn reset_hash(&mut self) {
    self.hash = None;
  }

  pub fn set_index(&mut self, index: i32) {
    self.index = Some(index);
  }

  pub fn with_index(mut self, index: i32) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.index = Some(index);
    self
  }

  pub fn index(&self) -> Option<&i32> {
    self.index.as_ref()
  }

  pub fn reset_index(&mut self) {
    self.index = None;
  }

  pub fn set_redacted(&mut self, redacted: bool) {
    self.redacted = Some(redacted);
  }

  pub fn with_redacted(mut self, redacted: bool) -> DestinyDefinitionsDestinyTalentGridDefinition {
    self.redacted = Some(redacted);
    self
  }

  pub fn redacted(&self) -> Option<&bool> {
    self.redacted.as_ref()
  }

  pub fn reset_redacted(&mut self) {
    self.redacted = None;
  }

}



